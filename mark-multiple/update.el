(let ((lisp-dir (concat zconfig-current-module-dir "/lisp"))
  ...
  (shell-command (concat "rm -rf " update-dir "/mark-multiple"))

  (let ((build-cmd (concat "cd " update-dir " && git clone git://github.com/magnars/mark-multiple.el.git mark-multiple"
			  " && cd mark-multiple"
			  " && rm -rf .git"
			  " && cp -r ./* ../../lisp/")))
